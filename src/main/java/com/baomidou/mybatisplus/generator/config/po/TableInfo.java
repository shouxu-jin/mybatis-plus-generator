...
public class TableInfo {
    ...
    private final List<TableField> addFields = new ArrayList<>();
    private final List<TableField> updateFields = new ArrayList<>();
    private final List<TableField> deleteFields = new ArrayList<>();
    private final List<TableField> paginationFields = new ArrayList<>();
    ...
    private List<String> entityNameRemovePrefixes = Arrays.asList("TFramework");
    ...
    public void importPackage() {
        try {
            String content = IOUtils.toString(getClass().getClassLoader().getResourceAsStream("importPackages.txt"), "UTF-8");
            for (String pack : content.split("\r\n")) {
                if (StringUtils.isNotBlank(pack)) {
                    this.importPackages.add(pack);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void processTable() {
        ...
        for (String entityNameRemovePrefix : entityNameRemovePrefixes) {
            entityName = entityName.replace(entityNameRemovePrefix, "");
        }
    ...
    public List<TableField> getUpdateFields() {
        return updateFields;
    }

    public List<TableField> getDeleteFields() {
        return deleteFields;
    }

    public List<TableField> getPaginationFields() {
