...
import com.baomidou.mybatisplus.generator.util.TableExtendProperty;
import com.baomidou.mybatisplus.generator.util.TableExtends;
...
public abstract class AbstractTemplateEngine {
    ...
    protected void outputVO(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) throws IOException, InvocationTargetException, IllegalAccessException, InstantiationException, NoSuchMethodException {
        String entityName = tableInfo.getEntityName();
        String entityPath = getPathInfo(OutputFile.vo);
        TableExtends tableExtends = new ObjectMapper().readValue(getClass().getClassLoader().getResourceAsStream("tables/" + tableInfo.getName() + ".json"), TableExtends.class);
        if (CollectionUtils.isNotEmpty(tableExtends.getAdd())) {
            for (TableExtendProperty tableExtendProperty : tableExtends.getAdd()) {
                for (TableField field : tableInfo.getFields()) {
                    if (StringUtils.equals(tableExtendProperty.getPropertyName(), field.getPropertyName())) {
                        tableInfo.getAddFields().add(field.clone(tableExtendProperty));
                    }
                }
            }
            String entityFile = String.format((entityPath + File.separator + "Add%sRequest" + suffixJavaOrKt()), entityName);
            outputFile(new File(entityFile), objectMap, "/templates/entity_add.java.ftl", getConfigBuilder().getStrategyConfig().entity().isFileOverride());
        }
        if (CollectionUtils.isNotEmpty(tableExtends.getDelete())) {
            for (TableExtendProperty tableExtendProperty : tableExtends.getDelete()) {
                for (TableField field : tableInfo.getFields()) {
                    if (StringUtils.equals(tableExtendProperty.getPropertyName(), field.getPropertyName())) {
                        tableInfo.getDeleteFields().add(field.clone(tableExtendProperty));
                    }
                }
            }
            String entityFile = String.format((entityPath + File.separator + "Delete%sRequest" + suffixJavaOrKt()), entityName);
            outputFile(new File(entityFile), objectMap, "/templates/entity_delete.java.ftl", getConfigBuilder().getStrategyConfig().entity().isFileOverride());
        }
        if (CollectionUtils.isNotEmpty(tableExtends.getUpdate())) {
            for (TableExtendProperty tableExtendProperty : tableExtends.getUpdate()) {
                for (TableField field : tableInfo.getFields()) {
                    if (StringUtils.equals(tableExtendProperty.getPropertyName(), field.getPropertyName())) {
                        tableInfo.getUpdateFields().add(field.clone(tableExtendProperty));
                    }
                }
            }
            String entityFile = String.format((entityPath + File.separator + "Update%sRequest" + suffixJavaOrKt()), entityName);
            outputFile(new File(entityFile), objectMap, "/templates/entity_update.java.ftl", getConfigBuilder().getStrategyConfig().entity().isFileOverride());
        }
        if (CollectionUtils.isNotEmpty(tableExtends.getPagination())) {
            for (TableExtendProperty tableExtendProperty : tableExtends.getPagination()) {
                for (TableField field : tableInfo.getFields()) {
                    if (StringUtils.equals(tableExtendProperty.getPropertyName(), field.getPropertyName())) {
                        tableInfo.getPaginationFields().add(field.clone(tableExtendProperty));
                    }
                }
            }
            String entityFile = String.format((entityPath + File.separator + "Pagination%sRequest" + suffixJavaOrKt()), entityName);
            outputFile(new File(entityFile), objectMap, "/templates/entity_pagination.java.ftl", getConfigBuilder().getStrategyConfig().entity().isFileOverride());
        }
        String entityFile = String.format((entityPath + File.separator + "Delete%sByIdRequest" + suffixJavaOrKt()), entityName);
        outputFile(new File(entityFile), objectMap, "/templates/entity_delete_id.java.ftl", getConfigBuilder().getStrategyConfig().entity().isFileOverride());
        entityFile = String.format((entityPath + File.separator + "Delete%sByIdsRequest" + suffixJavaOrKt()), entityName);
        outputFile(new File(entityFile), objectMap, "/templates/entity_delete_ids.java.ftl", getConfigBuilder().getStrategyConfig().entity().isFileOverride());
    }
